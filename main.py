...
def get_doc_owner_name(number):
    user_doc_number = number #input('Введите номер документа - ')
    print()
...
def append_doc_to_shelf(doc_number, shelf_number):
    ...
    return shelf_number, doc_number
...
def move_doc_to_shelf(d_number, s_number):
    user_doc_number = d_number #input('Введите номер документа - ')
    user_shelf_number = s_number #input('Введите номер полки для перемещения - ')
    remove_doc_from_shelf(user_doc_number)
    append_doc_to_shelf(user_doc_number, user_shelf_number)
    result = 'Документ номер "{}" был перемещен на полку номер "{}"'.format(user_doc_number, user_shelf_number)
    return result
...
def show_all_docs_info():
    ...
    doc_list = []
    for current_document in documents:
        item = show_document_info(current_document)
        doc_list.append(item)
    return doc_list


def add_new_doc(num, type, o_name, s_num):
    new_doc_number = num # input('Введите номер документа - ')
    new_doc_type = type #input('Введите тип документа - ')
    new_doc_owner_name = o_name #input('Введите имя владельца документа- ')
    new_doc_shelf_number = s_num #input('Введите номер полки для хранения - ')
    new_doc = {
        "type": new_doc_type,
    ...
    result = (new_doc, new_doc_shelf_number)
    return result
